...
@app_commands.describe(
    ...
    mensaje="Mensaje a enviar (usa Shift+Enter para saltos de línea, máx 2000 caracteres)"
)
@app_commands.checks.has_permissions(administrator=True)
async def send_message(interaction: discord.Interaction, canal: discord.TextChannel, mensaje: str):
    """Envía mensaje con formato multilínea"""
    
    # Verificar límite de caracteres de Discord
    if len(mensaje) > 2000:
        embed = discord.Embed(
            title="⚠️ Mensaje muy largo",
            description=f"El mensaje tiene {len(mensaje)} caracteres. El máximo permitido por Discord es 2000.",
            color=discord.Color.red()
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
