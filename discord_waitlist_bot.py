...
TIER_ROLES_POR_MODALIDAD = {
    ...
    # SMP, Axe, Dpot - configurar cuando crees los roles
}

TESTER_ROLE_ID = 1459287018746941615
CHITER_ROLE_ID = 1463405974466334793   # 🙅‍♂️┃Chiter (NUEVO)
...
COOLDOWN_DAYS = 10  # Cooldown normal (10 días)
MAX_QUEUE_SIZE = 20
...
# ==================== NUEVO: DETECTAR BOOSTERS ====================
@bot.event
async def on_member_update(before: discord.Member, after: discord.Member):
    """Detectar cuando alguien boostea el servidor"""
    # Verificar si empezó a boostear
    if not before.premium_since and after.premium_since:
        print(f"🎉 {after.name} boosteró el servidor!")
        
        # Obtener el rol de booster
        booster_role = after.guild.get_role(BOOSTER_ROLE_ID)
        
        if booster_role:
            # Dar el rol automáticamente
            try:
                await after.add_roles(booster_role)
                print(f"✅ Rol {booster_role.name} asignado a {after.name}")
            except Exception as e:
                print(f"❌ Error asignando rol booster: {e}")
            
            # Enviar mensaje de bienvenida al canal
            canal = bot.get_channel(BOOSTER_WELCOME_CHANNEL)
            if canal:
                embed = discord.Embed(
                    title="🎉 ¡Nuevo Booster!",
                    description=f"**¡Gracias {after.mention} por boostear el servidor!**",
                    color=discord.Color.from_rgb(255, 115, 250)  # Color rosa de booster
                )
                embed.add_field(
                    name="💎 Beneficio",
                    value=f"Ahora tienes el rol {booster_role.mention}",
                    inline=False
                )
                embed.add_field(
                    name="⏰ Cooldown Reducido",
                    value=f"Tu cooldown será de solo **{COOLDOWN_BOOSTER} días** en todas las modalidades (en vez de {COOLDOWN_DAYS} días)",
                    inline=False
                )
                ...
                try:
                    ...
                    print(f"✅ Mensaje de bienvenida enviado a {canal.name}")
                except Exception as e:
                    print(f"❌ Error enviando mensaje de bienvenida: {e}")
        else:
            print(f"⚠️ Rol booster (ID: {BOOSTER_ROLE_ID}) no encontrado")
...
def add_cooldown(user_id: str, mode: str, is_booster: bool = False):
    """Agrega cooldown para una modalidad específica (5 días si es booster, 10 si no)"""
    days = COOLDOWN_BOOSTER if is_booster else COOLDOWN_DAYS
    ...
    data['cooldowns'][user_id][mode] = {
        ...
        'is_booster': is_booster  # Guardar si era booster
    }
    save_data()
    ...
    if POSTGRESQL_AVAILABLE:
        if database.save_cooldown(user_id, mode, start_date, end_date):
            print(f"✅ Cooldown guardado en PostgreSQL: {user_id} - {mode} ({days} días)")
        else:
        ...
        if not testers:
            ...
            for t_id in testers:
                try:
                    ...
                    tester_mentions.append(user.mention)  # MENCIÓN REAL
                except:
                    pass
...
async def publicar_resultado(
    ...
    # NUEVO: Verificar si es booster para cooldown reducido
    is_booster = any(role.id == BOOSTER_ROLE_ID for role in jugador_discord.roles) if hasattr(jugador_discord, 'roles') else False
    
    # Si es Member (tiene roles), verificar booster
    member = interaction.guild.get_member(jugador_discord.id)
    if member:
        is_booster = any(role.id == BOOSTER_ROLE_ID for role in member.roles)
    
    end_date = add_cooldown(jugador_id, modo, is_booster)
    save_data()
    ...
    # DM de cooldown (MODIFICADO: indicar si es booster)
    try:
        dias_cooldown = COOLDOWN_BOOSTER if is_booster else COOLDOWN_DAYS
        
        cooldown_embed = discord.Embed(
            title=f"✅ ¡Gracias por testearte en {modo}!",
            description=f"Para volver a testearte en **{modo}** tendrás que esperar **{dias_cooldown} días**\n\n✨ Puedes testearte en otras modalidades sin esperar",
            color=discord.Color.blue()
        )
        ...
        if is_booster:
            cooldown_embed.set_footer(text="💎 Cooldown reducido por ser Booster")
...
async def banchiterlist(
    ...
    # NUEVO: Asignar rol 🙅‍♂️┃Chiter automáticamente
    try:
        guild = interaction.guild
        member = guild.get_member(jugador_discord.id)
        
        if member:
            chiter_role = guild.get_role(CHITER_ROLE_ID)
            if chiter_role:
                await member.add_roles(chiter_role)
                print(f"✅ Rol {chiter_role.name} asignado automáticamente a {member.name}")
            else:
                print(f"⚠️ Rol Chiter (ID: {CHITER_ROLE_ID}) no encontrado")
            
            # Remover todos los roles de tier
            for modalidad_roles in TIER_ROLES_POR_MODALIDAD.values():
                for tier_role_id in modalidad_roles.values():
                    role = guild.get_role(tier_role_id)
                    if role and role in member.roles:
                        await member.remove_roles(role)
            print(f"✅ Removidos roles de tier por ban: {motivo}")
    except Exception as e:
        print(f"❌ Error asignando rol chiter: {e}")
    ...
    for user_id, modes_data in list(cooldowns.items()):
        ...
        try:
            ...
            if isinstance(modes_data, dict) and 'end_date' in modes_data:
                ...
                for mode, cooldown_data in modes_data.items():
                    end_date = datetime.fromisoformat(cooldown_data['end_date'])
                    is_booster = cooldown_data.get('is_booster', False)
                    booster_text = " 💎" if is_booster else ""
                    modes_text += f"{MODE_EMOJIS.get(mode, '🎮')} {mode}: <t:{int(end_date.timestamp())}:R>{booster_text}\n"
...
# ==================== NUEVO COMANDO: /mensaje ====================
@bot.tree.command(name="mensaje", description="Envía un mensaje a un canal con formato multilínea")
@app_commands.describe(
    canal="Canal donde enviar el mensaje",
    mensaje="Mensaje a enviar. Usa \\n o | para saltos de línea. Ej: Hola\\nmundo o Hola|mundo"
)
@app_commands.checks.has_permissions(administrator=True)
async def send_message(interaction: discord.Interaction, canal: discord.TextChannel, mensaje: str):
    """Envía mensaje con formato multilínea (usa \\n o | para saltos de línea)"""
    
    # Convertir \n y | a saltos de línea reales
    mensaje_formateado = mensaje.replace('\\n', '\n').replace('|', '\n')
    
    # Verificar límite de caracteres de Discord
    if len(mensaje_formateado) > 2000:
        embed = discord.Embed(
            title="⚠️ Mensaje muy largo",
            description=f"El mensaje tiene {len(mensaje_formateado)} caracteres. El máximo permitido por Discord es 2000.",
            color=discord.Color.red()
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    ...
    try:
        ...
        embed = discord.Embed(
            ...
            description=f"Mensaje enviado correctamente a {canal.mention}",
            color=discord.Color.green()
        )
        
        # Vista previa con formato
        preview = mensaje_formateado[:100]
        if len(mensaje_formateado) > 100:
            preview += "..."
        ...
        embed.add_field(
            name="📊 Caracteres",
            value=f"{len(mensaje_formateado)}/2000",
            inline=True
        )
        embed.add_field(
            name="📏 Líneas",
            value=f"{mensaje_formateado.count(chr(10)) + 1}",
            inline=True
        )
...
# COMANDO: /add (añadir usuario a ticket)
...
# COMANDO: /addtester (añadir tester a ticket)
