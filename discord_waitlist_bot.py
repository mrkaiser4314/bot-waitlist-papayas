...
# === ROLES DE TESTER POR MODALIDAD ===
TESTER_ROLES_POR_MODALIDAD = {
    'NethOP': 1463707781516230666,    # Tester┃ NETHPOT
    'Crystal': 1463707424975491299,   # Tester ┃Crystal
    'SMP': 1466589220301963276,       # Tester┃ Smp
    'UHC': 1463707657994113024,       # Tester ┃UHC
    'Dpot': 1466589265730211967,      # Tester┃ Diamond Pot
    'Axe': 1466589258109292810,       # Tester┃ Axe
    'Mace': 1463928890652364891,      # Tester ┃MACE
    'Sword': 1464020629375287450      # Tester┃ Sword
}
...
# === FUNCIÓN HELPER: Verificar si usuario es tester de modalidad específica ===
def is_tester_of_mode(user_roles, modo):
    """
    Verifica si el usuario tiene el rol de tester de la modalidad específica
    
    Args:
        user_roles: Lista de roles del usuario (interaction.user.roles)
        modo: Modalidad a verificar (ej: 'Mace', 'Sword', etc.)
    
    Returns:
        bool: True si tiene el rol de tester de esa modalidad, False si no
    """
    # Verificar si tiene el rol de tester de esta modalidad
    tester_role_id = TESTER_ROLES_POR_MODALIDAD.get(modo)
    if tester_role_id:
        return any(role.id == tester_role_id for role in user_roles)
    
    # Si no hay rol específico configurado, usar rol general (backwards compatibility)
    return any(role.id == TESTER_ROLE_ID for role in user_roles)
...
class WaitlistView(discord.ui.View):
    ...
    async def tester_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        # Verificar que sea tester de ESTA modalidad específica
        if not is_tester_of_mode(interaction.user.roles, self.modo):
            await interaction.response.send_message(
                f"❌ Solo los testers de **{self.modo}** pueden usar este botón", 
                ephemeral=True
            )
            return
    ...
    async def toggle_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not is_tester_of_mode(interaction.user.roles, self.modo):
            await interaction.response.send_message(
                f"❌ Solo los testers de **{self.modo}** pueden abrir/cerrar esta waitlist", 
                ephemeral=True
            )
            return
...
@bot.tree.command(name="toptester", description="Ver el top de testers global y del mes por modalidad")
...
class TopTesterView(discord.ui.View):
    ...
    @discord.ui.select(
        ...
        options=[
            discord.SelectOption(label="Overall (Global)", value="Overall", emoji="🏆", description="Todos los modos combinados"),
            discord.SelectOption(label="Mace", value="Mace", emoji="🔨", description="Tests de Mace"),
            discord.SelectOption(label="Sword", value="Sword", emoji="⚔️", description="Tests de Sword"),
            discord.SelectOption(label="UHC", value="UHC", emoji="❤️", description="Tests de UHC"),
            discord.SelectOption(label="Crystal", value="Crystal", emoji="💎", description="Tests de Crystal"),
            discord.SelectOption(label="NethOP", value="NethOP", emoji="🧪", description="Tests de NethOP"),
            discord.SelectOption(label="SMP", value="SMP", emoji="🪓", description="Tests de SMP"),
            discord.SelectOption(label="Axe", value="Axe", emoji="🪓", description="Tests de Axe"),
            discord.SelectOption(label="Dpot", value="Dpot", emoji="🧪", description="Tests de Dpot"),
        ]
    )
    async def select_mode(self, interaction: discord.Interaction, select: discord.ui.Select):
        ...
        # Crear embed con stats del modo seleccionado
        embed = await create_toptester_embed(mode)
        
        # Actualizar mensaje
        await interaction.edit_original_response(embed=embed, view=self)

async def create_toptester_embed(mode: str):
    """Crea el embed de top testers para el modo especificado"""
    
    # Obtener stats de testers
    tester_counts_global = {}  # Todos los tiempos
    tester_counts_month = {}   # Solo este mes
    ...
    for resultado in data.get('resultados', []):
        ...
        # Filtrar por modo si no es Overall
        if mode != 'Overall' and modalidad != mode:
            continue
        
        if tester_id:
            # Stats globales
            if tester_id not in tester_counts_global:
                tester_counts_global[tester_id] = {
                    'name': tester_name,
                    'count': 0
                }
            tester_counts_global[tester_id]['count'] += 1
            ...
            if fecha_str:
                try:
                    ...
                    if fecha.month == current_month and fecha.year == current_year:
                        if tester_id not in tester_counts_month:
                            tester_counts_month[tester_id] = {
                                'name': tester_name,
                                'count': 0
                            }
                        tester_counts_month[tester_id]['count'] += 1
                except:
                    pass
    
    # Crear embed
    mode_emoji = MODE_EMOJIS.get(mode, '🏆')
    title = f"{mode_emoji} Top Testers Global" if mode == 'Overall' else f"{mode_emoji} Top Testers - {mode}"
    ...
    # Top Global (todos los tiempos)
    if tester_counts_global:
        sorted_global = sorted(tester_counts_global.items(), key=lambda x: x[1]['count'], reverse=True)
        ...
        embed.add_field(
            name="🌍 Top Global",
            value=top_global_text if top_global_text else "Sin datos",
            inline=False
        )
    else:
        embed.add_field(
    ...
    # Top del Mes
    if tester_counts_month:
        sorted_month = sorted(tester_counts_month.items(), key=lambda x: x[1]['count'], reverse=True)
        ...
        embed.add_field(
            ...
            value=top_month_text if top_month_text else "Sin datos este mes",
            inline=False
        )
    else:
        embed.add_field(
            name="📅 Top del mes",
            value="Sin datos este mes",
            inline=False
        )
    
    # Footer con actualización automática
    embed.set_footer(text="Datos actualizados automáticamente cada 5 minutos • hoy a las 17:29")
